...
static float pi(void) {return 3.14159265358979323846;}
static float e(void) {return 2.71828182845904523536;}
...
static float lessthan(float a, float b){
  if(a<b)
  {
    return 1;
  }
...
static float greaterthan(float a, float b){
  if(a>b)
  {
    return 1;
  }
    ...
    do {
        ...
        if ((s->next[0] >= '0' && s->next[0] <= '9') || s->next[0] == '.') {
            ...
            if (isalpha(s->next[0])) {
                ...
                const te_variable *var = find_lookup(s, start, tok_len);
                if (!var) var = find_builtin(start, tok_len);
                ...
                switch (s->next++[0]) {
                    ...
                    case '<': s->type = TOK_INFIX; s->function = lessthan; break;
                    case '>': s->type = TOK_INFIX; s->function = greaterthan; break;
...
float te_eval(const te_expr *n) {
    ...
    switch(TYPE_MASK(n->type)) {
        ...
        case TE_VARIABLE: return te_read_var(n->bound);
