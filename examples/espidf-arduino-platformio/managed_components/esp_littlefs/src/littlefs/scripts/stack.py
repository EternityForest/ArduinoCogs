...
# size results
class StackResult(
    co.namedtuple("StackResult", ["file", "function", "frame", "limit", "children"])
):
    _by = ["file", "function"]
    _fields = ["frame", "limit"]
    _sort = ["limit", "frame"]
    ...
    def __new__(cls, file="", function="", frame=0, limit=0, children=set()):
        return super().__new__(cls, file, function, Int(frame), Int(limit), children)
...
def collect(ci_paths, *, sources=None, everything=False, **args):
            ...
            for k, info in graph:
                if k == "node":
                    ...
                    if m_:
                        ...
                        callgraph[info["title"]] = (file, function, int(size), targets)
                elif k == "edge":
                    info = dict(info)
    ...
    for source, (s_file, s_function, frame, targets) in callgraph.items():
        ...
        if sources is not None:
            if not any(os.path.abspath(s_file) == os.path.abspath(s) for s in sources):
                continue
        else:
            # default to only cwd
            if (
                not everything
                and not os.path.commonpath([os.getcwd(), os.path.abspath(s_file)])
                == os.getcwd()
            ):
                continue

        # smiplify path
        if os.path.commonpath([os.getcwd(), os.path.abspath(s_file)]) == os.getcwd():
    ...
    if not everything:
        ...
        for source, (s_file, s_function, frame, targets) in callgraph.items():
            ...
            if sources is not None and not any(
                os.path.abspath(s_file) == os.path.abspath(s) for s in sources
            ):
                continue
            # discard internal functions
            ...
            for i, name in enumerate(names_):
                ...
                if not tree:
                    print(
                        ...
                        % (
                            " ".join(
                                "%*s" % (w, x) for w, x in zip(widths[1:], line[1:-1])
                            ),
                            line[-1],
                        ),
                        end="",
...
def main(ci_paths, by=None, fields=None, defines=None, sort=None, **args):
    ...
    if not args.get("use", None):
        ...
        with openio(args["use"]) as f:
            ...
            for r in reader:
                if not any(
                    "stack_" + k in r and r["stack_" + k].strip()
    ...
    if args.get("output"):
        with openio(args["output"], "w") as f:
            writer = csv.DictWriter(
                ...
                + [
                    "stack_" + k
                    for k in (fields if fields is not None else StackResult._fields)
                ],
            )
            writer.writeheader()
            for r in results:
    ...
    # error on recursion
    if args.get("error_on_recursion") and any(m.isinf(float(r.limit)) for r in results):
        sys.exit(2)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Find stack usage at the function level.", allow_abbrev=False
    )
    parser.add_argument("ci_paths", nargs="*", help="Input *.ci files.")
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
    ...
    parser.add_argument(
        "--tree", action="store_true", help="Only show the function call tree."
    )
    parser.add_argument(
        "-Z",
        "--depth",
