...
# data size results
class DataResult(co.namedtuple("DataResult", ["file", "function", "size"])):
    _by = ["file", "function"]
    _fields = ["size"]
    _sort = ["size"]
    _types = {"size": Int}
    ...
    def __new__(cls, file="", function="", size=0):
        return super().__new__(cls, file, function, Int(size))

    def __add__(self, other):
        return DataResult(self.file, self.function, self.size + other.size)
        ...
        for line in proc.stdout:
            ...
            if m:
                ...
                if not everything and func.startswith("__"):
                    continue
                results_.append(DataResult(file, func, int(m.group("size"), 16)))
        proc.wait()
...
def main(obj_paths, *, by=None, fields=None, defines=None, sort=None, **args):
    # find sizes
    ...
    if args.get("output"):
        with openio(args["output"], "w") as f:
            writer = csv.DictWriter(
                ...
                + [
                    "data_" + k
                    for k in (fields if fields is not None else DataResult._fields)
                ],
            )
            writer.writeheader()
            for r in results:
    ...
    if args.get("diff"):
        ...
        try:
            with openio(args["diff"]) as f:
                ...
                for r in reader:
                    if not any(
                        "data_" + k in r and r["data_" + k].strip()
                        for k in DataResult._fields
                    ):
                        continue
                    try:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Find data size at the function level.", allow_abbrev=False
    )
    parser.add_argument("obj_paths", nargs="*", help="Input *.o files.")
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
