...
class StructResult(co.namedtuple("StructResult", ["file", "struct", "size"])):
    ...
    def __new__(cls, file="", struct="", size=0):
        return super().__new__(cls, file, struct, Int(size))

    def __add__(self, other):
        return StructResult(self.file, self.struct, self.size + other.size)
    ...
    for path in obj_paths:
        ...
        for r in results_:
            ...
            if sources is not None:
                if not any(
                    os.path.abspath(r.file) == os.path.abspath(s) for s in sources
                ):
                    continue
            else:
                # default to only cwd
                if (
                    not everything
                    and not os.path.commonpath([os.getcwd(), os.path.abspath(r.file)])
                    == os.getcwd()
                ):
                    continue
            ...
            # simplify path
            if (
                os.path.commonpath([os.getcwd(), os.path.abspath(r.file)])
                == os.getcwd()
            ):
...
def main(obj_paths, *, by=None, fields=None, defines=None, sort=None, **args):
    ...
    if not args.get("use", None):
        ...
        with openio(args["use"]) as f:
            ...
            for r in reader:
                if not any(
                    "struct_" + k in r and r["struct_" + k].strip()
                ...
                try:
                    results.append(
                        StructResult(
                            **{
                                k: r[k]
                                for k in StructResult._by
                                if k in r and r[k].strip()
                            },
                            **{
                                k: r["struct_" + k]
                                for k in StructResult._fields
                                if "struct_" + k in r and r["struct_" + k].strip()
                            },
                        )
                    )
                except TypeError:
    ...
    if args.get("output"):
        with openio(args["output"], "w") as f:
            writer = csv.DictWriter(
                ...
                + [
                    "struct_" + k
                    for k in (fields if fields is not None else StructResult._fields)
                ],
            )
            writer.writeheader()
            for r in results:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Find struct sizes.", allow_abbrev=False
    )
    ...
    parser.add_argument(
        "--internal", action="store_true", help="Also show structs in .c files."
    )
    parser.add_argument(
        "--objdump-path",
