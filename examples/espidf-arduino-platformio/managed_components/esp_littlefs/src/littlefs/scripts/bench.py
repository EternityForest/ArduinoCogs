        ...
        with openio(args["output"], "w") as f:
            ...
            def write_case_functions(f, suite, case):
                    ...
                    for i, defines in enumerate(case.permutations):
                        for k, v in sorted(defines.items()):
                            if v not in define_cbs:
                                name = "__bench__%s__%s__%d" % (case.name, k, i)
                                define_cbs[v] = name
                                f.writeln(
                                    "intmax_t %s("
                ...
                if suite.if_ is not None or case.if_ is not None:
                    f.writeln("bool __bench__%s__filter(void) {" % (case.name))
                    f.writeln(
                        4 * " "
                        + "return %s;"
                        % " && ".join(
            ...
            if not args.get("source"):
                ...
                f.writeln('__attribute__((section("_bench_suites"), ' "aligned(1)))")
                f.writeln(
                    "const struct bench_suite __bench__%s__suite = {" % suite.name
                )
                ...
                for case in suite.cases:
                    ...
                    if suite.if_ is not None or case.if_ is not None:
                        f.writeln(
                            12 * " " + ".filter = __bench__%s__filter," % (case.name)
                        )
                    f.writeln(12 * " " + ".run = __bench__%s__run," % (case.name))
                    f.writeln(8 * " " + "},")
                f.writeln(4 * " " + "},")
                        ...
                        if case.in_ is not None and os.path.normpath(
                            ...
                            if suite.defines:
                                for i, define in enumerate(sorted(suite.defines)):
                                    ...
                                    f.writeln(
                                        "#define " "__BENCH__%s__NEEDS_UNDEF" % (define)
                                    )
                                    f.writeln("#endif")
                                f.writeln()
                            ...
                            if suite.defines:
                                for define in sorted(suite.defines):
                                    f.writeln(
                                        "#ifdef __BENCH__%s__NEEDS_UNDEF" % define
                                    )
                                    f.writeln(
                                        "#undef __BENCH__%s__NEEDS_UNDEF" % define
                                    )
...
def run_stage(name, runner_, ids, stdout_, trace_, output_, **args):
    ...
    def run_runner(runner_, ids=[]):
        ...
        proc.wait()
        if proc.returncode != 0:
            raise BenchFailure(last_id, proc.returncode, list(last_stdout), last_assert)
        ...
        while start < total_perms:
            ...
            try:
                ...
                if output_:
                    ...
                    output_.writerow({"suite": suite, "case": case, **defines})
    ...
    return (expected_perms, passed_perms, readed, proged, erased, failures, killed)
...
def run(runner, bench_ids=[], **args):
    ...
    (_, expected_suite_perms, expected_case_perms, expected_perms, total_perms) = (
        find_perms(runner_, bench_ids, **args)
    )
    print(
    ...
    for by in (
        ...
        (expected_, passed_, readed_, proged_, erased_, failures_, killed) = run_stage(
            by or "benches",
            runner_,
    ...
    print(
        "%sdone:%s %s"
        % (
            ("\x1b[34m" if not failures else "\x1b[31m") if args["color"] else "",
            "\x1b[m" if args["color"] else "",
            ", ".join(
                filter(
                    None,
    ...
    # drop into gdb?
    if failures and (args.get("gdb") or args.get("gdb_case") or args.get("gdb_main")):
        failure = failures[0]
...
if __name__ == "__main__":
    ...
    bench_parser.add_argument(
        "bench_ids", nargs="*", help="Description of benches to run."
    )
    bench_parser.add_argument(
        "-Y", "--summary", action="store_true", help="Show quick summary."
    )
    bench_parser.add_argument(
        "-l", "--list-suites", action="store_true", help="List bench suites."
    )
    bench_parser.add_argument(
        "-L", "--list-cases", action="store_true", help="List bench cases."
    )
    bench_parser.add_argument(
        "--list-suite-paths",
        action="store_true",
    ...
    bench_parser.add_argument(
        "-D", "--define", action="append", help="Override a bench define."
    )
    bench_parser.add_argument(
        "-G", "--geometry", help="Comma-separated list of disk geometries to bench."
    )
    bench_parser.add_argument(
        "-d", "--disk", help="Direct block device operations to this file."
    )
    bench_parser.add_argument("-t", "--trace", help="Direct trace output to this file.")
    bench_parser.add_argument(
        "--trace-backtrace",
        action="store_true",
    ...
    bench_parser.add_argument("-o", "--output", help="CSV file to store results.")
    bench_parser.add_argument("--read-sleep", help="Artificial read delay in seconds.")
    bench_parser.add_argument("--prog-sleep", help="Artificial prog delay in seconds.")
    bench_parser.add_argument(
        "--erase-sleep", help="Artificial erase delay in seconds."
    )
    bench_parser.add_argument(
        "-j",
        "--jobs",
    ...
    bench_parser.add_argument(
        "-b", "--by-suites", action="store_true", help="Step through benches by suite."
    )
    bench_parser.add_argument(
        "-B", "--by-cases", action="store_true", help="Step through benches by case."
    )
    bench_parser.add_argument(
        "--context",
        type=lambda x: int(x, 0),
        default=5,
        help="Show this many lines of stdout on bench failure. " "Defaults to 5.",
    )
    bench_parser.add_argument(
        "--gdb", action="store_true", help="Drop into gdb on bench failure."
    )
    bench_parser.add_argument(
        "--gdb-case",
        action="store_true",
    ...
    bench_parser.add_argument(
        "--gdb-main",
        action="store_true",
        help="Drop into gdb on bench failure but stop at the beginning " "of main.",
    )
    bench_parser.add_argument(
        "--gdb-path",
        type=lambda x: x.split(),
