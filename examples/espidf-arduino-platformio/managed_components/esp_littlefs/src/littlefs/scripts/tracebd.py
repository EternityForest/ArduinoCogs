...
def hilbert_curve(width, height):
    ...
    def hilbert_(x, y, a_x, a_y, b_x, b_y):
        ...
        if 2 * w > 3 * h:
            ...
            yield from hilbert_(
                x + (a_x - a_dx) + (b_x_ - b_dx),
                y + (a_y - a_dy) + (b_y_ - b_dy),
                -b_x_,
                -b_y_,
                -(a_x - a_x_),
                -(a_y - a_y_),
...
class Block(int):
    ...
    def __new__(cls, state=0, *, wear=0, readed=False, proged=False, erased=False):
    ...
    def __or__(self, other):
        return Block((int(self) | int(other)) & 7, wear=max(self.wear, other.wear))

    def worn(self, max_wear, *, block_cycles=None, wear_chars=None, **_):
    ...
    def draw(
        ...
        **_,
        ...
        if wear:
            w = min(
                self.worn(max_wear, block_cycles=block_cycles, wear_chars=wear_chars), 1
            )
        ...
        # apply colors
        if f and color:
            c = "%s%s\x1b[m" % ("".join("\x1b[%sm" % f_ for f_ in f), c)

        return c


class Bd:
    def __init__(self, *, size=1, count=1, width=None, height=1, blocks=None):
        if width is None:
            width = count
    ...
    def _op(self, f, block=None, off=None, size=None):
        if block is None:
            ...
            self.resize(
                size=max(self.size, off + size), count=max(self.count, block + 1)
            )
            ...
            range_ = range(
                m.floor(start * len(self.blocks)), m.ceil(stop * len(self.blocks))
            )
    ...
    def resize(self, *, size=None, count=None, width=None, height=None):
        ...
        for x in range(width * height):
            ...
            # aggregate state
            blocks.append(ft.reduce(Block.__or__, self.blocks[start:stop], Block()))
    ...
    def draw(
        ...
        **args,
        ...
        if hilbert:
            ...
            for (x, y), b in zip(hilbert_curve(self.width, self.height), self.blocks):
                grid[x + y * self.width] = b
        elif lebesgue:
            grid = [None] * (self.width * self.height)
            for (x, y), b in zip(lebesgue_curve(self.width, self.height), self.blocks):
                grid[x + y * self.width] = b
        else:
            grid = self.blocks
        ...
        if (braille and self.height < 4) or (dots and self.height < 2):
            ...
            grid = list(
                it.chain.from_iterable(
                    # did we resize?
                    it.islice(it.chain(h, it.repeat(Block())), self.width * self.height)
        ...
        if braille:
            ...
            for x in range(self.width):
                line.append(
                    grid[x + row * self.width].draw(
                        max_wear, read=read, prog=prog, erase=erase, wear=wear, **args
                    )
                )
    ...
    **args,
    ...
    def resize(*, size=None, count=None):
        ...
        bd.resize(
            ...
            height=max(1, 4 * height_ if braille else 2 * height_ if dots else height_),
        )
    ...
    def parse(line):
        ...
        if m.group("create"):
            ...
            if reset:
                bd = Bd(size=bd.size, count=bd.count, width=bd.width, height=bd.height)
            return True
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Display operations on block devices based on " "trace output.",
        allow_abbrev=False,
    )
    parser.add_argument("path", nargs="?", help="Path to read from.")
    parser.add_argument("-r", "--read", action="store_true", help="Render reads.")
    parser.add_argument("-p", "--prog", action="store_true", help="Render progs.")
    parser.add_argument("-e", "--erase", action="store_true", help="Render erases.")
    parser.add_argument("-w", "--wear", action="store_true", help="Render wear.")
    parser.add_argument(
        "-b",
        "--block",
        type=lambda x: tuple(int(x, 0) if x.strip() else None for x in x.split(",")),
    ...
    parser.add_argument(
        "--block-count", type=lambda x: int(x, 0), help="Assume a specific block count."
    )
    parser.add_argument(
        "-C",
        "--block-cycles",
    ...
    parser.add_argument(
        "-:", "--dots", action="store_true", help="Use 1x2 ascii dot characters."
    )
    parser.add_argument(
        "-⣿",
        "--braille",
    ...
    parser.add_argument(
        "--chars", help="Characters to use for read, prog, erase, noop operations."
    )
    parser.add_argument("--wear-chars", help="Characters to use for showing wear.")
    parser.add_argument(
        "--colors",
    ...
    parser.add_argument(
        ...
        "--keep-open",
        action="store_true",
        help="Reopen the pipe on EOF, useful when multiple " "processes are writing.",
    )
    sys.exit(
        main(
            **{
