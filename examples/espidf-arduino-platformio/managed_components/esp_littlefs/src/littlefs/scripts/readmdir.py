...
class Tag:
    ...
    def is_(self, type):
        try:
            if " " in type:
                ...
                return self.is_(type1) and (self.type & ~TAG_TYPES[type1][0]) == int(
                    type3, 0
                )
    ...
    def mkmask(self):
        return Tag(0x700 if self.isunique else 0x7FF, 0x3FF if self.isattr else 0, 0)
    ...
    def chid(self, nid):
        ...
        if hasattr(self, "data"):
            ntag.data = self.data
        if hasattr(self, "ccrc"):
            ntag.crc = self.crc
        if hasattr(self, "erased"):
            ntag.erased = self.erased
        return ntag

    def typerepr(self):
        if self.is_("ccrc") and getattr(self, "ccrc", 0xFFFFFFFF) != 0xFFFFFFFF:
            crc_status = " (bad)"
...
class MetadataPair:
    def __init__(self, blocks):
            ...
            if tag.is_("fcrc") and len(tag.data) == 8:
                ...
                if not corrupt:
                    ...
                    if fcrcdata:
                        ...
                        fcrc_ = 0xFFFFFFFF ^ binascii.crc32(block[off : off + fcrcsize])
                        if fcrc_ == fcrc:
                            fcrctag.erased = True
                            corrupt = True
        ...
        # find active ids
        self.ids = list(it.takewhile(lambda id: Tag("name", id, 0) in self, it.count()))
    ...
    def __getitem__(self, args):
        ...
        for tag in reversed(self.log):
            if gmask.id != 0 and tag.is_("splice") and tag.id <= gtag.id - gdiff:
    ...
    def _dump_tags(self, tags, f=sys.stdout, truncate=True):
        f.write("%-8s  %-8s  %-13s %4s %4s" % ("off", "tag", "type", "id", "len"))
        if truncate:
        ...
        for tag in tags:
            f.write(
                "%08x: %08x  %-14s %3s %4s"
                % (tag.off, tag, tag.typerepr(), tag.idrepr(), tag.sizerepr())
            )
            if truncate:
                f.write(
                    "  %-23s  %-8s\n"
...
def main(args):
    ...
    with open(args.disk, "rb") as f:
        for block in [args.block1, args.block2]:
            ...
            blocks.append(f.read(args.block_size).ljust(args.block_size, b"\xff"))
    ...
    print(
        "mdir {%s} rev %d%s%s%s"
        % (
            ", ".join("%#x" % b for b in [args.block1, args.block2] if b is not None),
            mdir.rev,
...
if __name__ == "__main__":
    ...
    parser.add_argument("disk", help="File representing the block device.")
    parser.add_argument(
        "block_size", type=lambda x: int(x, 0), help="Size of a block in bytes."
    )
    parser.add_argument(
        "block1",
    ...
    parser.add_argument("-l", "--log", action="store_true", help="Show tags in log.")
    parser.add_argument(
        "-a",
        "--all",
        action="store_true",
