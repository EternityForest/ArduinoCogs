...
def changeprefix(from_prefix, to_prefix, line):
    line, count1 = re.subn("\\b" + from_prefix, to_prefix, line)
    line, count2 = re.subn("\\b" + from_prefix.upper(), to_prefix.upper(), line)
    line, count3 = re.subn(
        "\\B-D" + from_prefix.upper(), "-D" + to_prefix.upper(), line
    )
    return line, count1 + count2 + count3


def changefile(from_prefix, to_prefix, from_path, to_path, *, no_replacements=False):
    ...
    print(
        "%s: %d replacements"
        % ("%s -> %s" % (from_path, to_path) if not to_path_temp else from_path, count)
    )
    ...
    for from_path in paths:
        ...
        if output:
            ...
            to_path = os.path.join(
                ...
                changeprefix(from_prefix, to_prefix, os.path.basename(from_path))[0],
            )

        # rename contents
        changefile(
            from_prefix, to_prefix, from_path, to_path, no_replacements=no_replacements
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("from_prefix", help="Prefix to replace.")
    parser.add_argument("to_prefix", help="Prefix to replace with.")
    parser.add_argument("paths", nargs="*", help="Files to operate on.")
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
    ...
    parser.add_argument(
        "-R", "--no-renames", action="store_true", help="Don't rename files"
    )
    parser.add_argument(
        "--git", action="store_true", help="Use git to find/update files."
    )
    parser.add_argument(
        "--no-stage", action="store_true", help="Don't stage changes with git."
    )
    parser.add_argument(
        "--git-path",
        type=lambda x: x.split(),
        default=GIT_PATH,
        help="Path to git executable, may include flags. " "Defaults to %r." % GIT_PATH,
    )
    sys.exit(
        main(
            **{
