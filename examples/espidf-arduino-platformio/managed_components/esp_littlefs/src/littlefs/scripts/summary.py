...
OPS = {
    ...
    "stddev": lambda xs: (
        lambda mean: Float(m.sqrt(sum((float(x) - mean) ** 2 for x in xs) / len(xs)))
...
# available types
TYPES = co.OrderedDict([("int", Int), ("float", Float), ("frac", Frac)])


def infer(results, *, by=None, fields=None, types={}, ops={}, renames=[], **_):
    ...
    def __add__(self, other):
        return self.__class__(
            ...
            **{
                k: object.__getattribute__(self, k) + object.__getattribute__(other, k)
                for k in fields
            },
        )
    ...
    return type(
        "Result",
        (co.namedtuple("Result", by + fields),),
        {
            "__slots__": (),
            "__new__": __new__,
            "__add__": __add__,
...
def main(csv_paths, *, by=None, fields=None, defines=None, sort=None, **args):
    # separate out renames
    renames = list(
        it.chain.from_iterable(
            ((k, v) for v in vs) for k, vs in it.chain(by or [], fields or [])
        )
    )
    if by is not None:
        by = [k for k, _ in by]
    ...
    # homogenize
    Result = infer(results, by=by, fields=fields, types=types, ops=ops, renames=renames)
    results_ = []
    for r in results:
        if not any(k in r and r[k].strip() for k in Result._fields):
            continue
        try:
            results_.append(
                Result(
    ...
    if args.get("output"):
        with openio(args["output"], "w") as f:
            ...
            for r in results:
                ...
                writer.writerow({k: getattr(r, k) for k in Result._by + Result._fields})
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Summarize measurements in CSV files.", allow_abbrev=False
    )
    ...
    parser.add_argument("--int", action="append", help="Treat these fields as ints.")
    parser.add_argument(
        "--float", action="append", help="Treat these fields as floats."
    )
    parser.add_argument(
        "--frac", action="append", help="Treat these fields as fractions."
    )
    parser.add_argument(
        "--sum", action="append", help="Add these fields (the default)."
    )
    parser.add_argument("--prod", action="append", help="Multiply these fields.")
    parser.add_argument(
        "--min", action="append", help="Take the minimum of these fields."
    )
    parser.add_argument(
        "--max", action="append", help="Take the maximum of these fields."
    )
    parser.add_argument("--mean", action="append", help="Average these fields.")
    parser.add_argument(
        "--stddev", action="append", help="Find the standard deviation of these fields."
    )
    parser.add_argument(
        "--gmean", action="append", help="Find the geometric mean of these fields."
    )
    parser.add_argument(
        "--gstddev",
        action="append",
