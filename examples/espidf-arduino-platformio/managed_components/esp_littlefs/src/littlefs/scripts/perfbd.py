...
class PerfBdResult(
    co.namedtuple(
        "PerfBdResult",
        ["file", "function", "line", "readed", "proged", "erased", "children"],
    )
):
    ...
    def __new__(
        cls, file="", function="", line=0, readed=0, proged=0, erased=0, children=[]
    ):
...
def collect_job(
    ...
    **args,
    ...
    trace_pattern = re.compile(
        ...
        r"\s*,"
        r"\s*(?P<erase_block>\w+)"
        r"\s*\(\s*(?P<erase_size>\w+)\s*\)"
        r"\s*\)"
        r")\s*$"
    )
    frame_pattern = re.compile(r"^\s+at (?P<addr>\w+)\s*$")
    ...
    def commit():
        ...
        if not last_stack:
            ...
            if sources is not None:
                if not any(
                    os.path.abspath(file) == os.path.abspath(s) for s in sources
                ):
                    return
            else:
                # default to only cwd
                if (
                    not everything
                    and not os.path.commonpath([os.getcwd(), os.path.abspath(file)])
                    == os.getcwd()
                ):
                    return

            # simplify path
            if os.path.commonpath([os.getcwd(), os.path.abspath(file)]) == os.getcwd():
                file = os.path.relpath(file)
            else:
                file = os.path.abspath(file)

            results[(file, sym, line)] = (last_readed, last_proged, last_erased, {})
        else:
            ...
            if not line.startswith("\t"):
                ...
                if m:
                    ...
                    if last_delta is None:
                        i = bisect.bisect(
                            lines, (last_file, last_line), key=lambda x: (x[0], x[1])
                        )
                        if i > 0:
    ...
    def to_results(results):
        ...
        for name, (r, p, e, children) in results.items():
            results_.append(PerfBdResult(*name, r, p, e, children=to_results(children)))
        return results_
...
def collect(obj_path, trace_paths, *, jobs=None, **args):
    ...
    if jobs is not None:
        ...
        with mp.Pool(jobs) as p:
            for results_ in p.imap_unordered(
                ...
                (
                    (
                        collect_job,
                        (path, start, stop, syms, sym_at, lines, line_at),
                        args,
                    )
        ...
        for path in trace_paths:
            results.extend(
                collect_job(path, None, None, syms, sym_at, lines, line_at, **args)
            )
    ...
    for path in co.OrderedDict.fromkeys(r.file for r in results).keys():
        ...
        if not annotate:
            ...
            for line, r in sorted(table.items()):
                if (
                    float(r.readed) / max_readed >= read_t0
        ...
        with open(path) as f:
            ...
            for i, line in enumerate(f):
                ...
                if i + 1 in table:
                    ...
                    if args["color"]:
                        if (
                            float(r.readed) / max_readed >= read_t1
    ...
    **args,
    ...
    if not args.get("use", None):
        ...
        with openio(args["use"]) as f:
            ...
            for r in reader:
                if not any(
                    "perfbd_" + k in r and r["perfbd_" + k].strip()
    ...
    if args.get("output"):
        with openio(args["output"], "w") as f:
            writer = csv.DictWriter(
                ...
                + [
                    "perfbd_" + k
                    for k in (fields if fields is not None else PerfBdResult._fields)
                ],
            )
            writer.writeheader()
            for r in results:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "obj_path", nargs="?", help="Input executable for mapping addresses to symbols."
    )
    parser.add_argument("trace_paths", nargs="*", help="Input *.trace files.")
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
