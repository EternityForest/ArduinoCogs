...
class TestCase:
    ...
    def __init__(self, config, args={}):
        ...
        self.in_ = config.pop("in", config.pop("suite_in", None))

        self.reentrant = config.pop("reentrant", config.pop("suite_reentrant", False))
        ...
        def parse_define(v):
            ...
            # or a string
                ...
                for v_ in csplit(v):
                    ...
                    if m:
                        start = int(m.group("start"), 0) if m.group("start") else 0
                        stop = int(m.group("stop"), 0) if m.group("stop") else None
                        step = int(m.group("step"), 0) if m.group("step") else 1
                        if m.lastindex <= 1:
                            start, stop = 0, start
                        for x in range(start, stop, step):
                            yield from parse_define(
                                "%s(%d)%s" % (v_[: m.start()], x, v_[m.end() :])
                            )
                    else:
                        yield v_
            # or a literal value
    ...
    def __init__(self, path, args={}):
        ...
        with open(self.path) as f:
            ...
            for (lineno, name), (nlineno, _) in it.zip_longest(
                case_linenos, case_linenos[1:], fillvalue=(float("inf"), None)
            ):
                code_lineno = min(
            ...
            self.code_lineno = min(
                (l for l in code_linenos if not case_linenos or l < case_linenos[0][0]),
                default=None,
            )
            ...
            self.cases = []
            for name, case in sorted(cases.items(), key=lambda c: c[1].get("lineno")):
                self.cases.append(
                    TestCase(
                        config={
                            "name": name,
                            "path": path
                            + (":%d" % case["lineno"] if "lineno" in case else ""),
                            "suite": self.name,
            ...
            self.defines = set.union(*(set(case.defines) for case in self.cases))
        ...
        with openio(args["output"], "w") as f:
            ...
            def write_case_functions(f, suite, case):
                    ...
                    for i, defines in enumerate(case.permutations):
                        for k, v in sorted(defines.items()):
                            if v not in define_cbs:
                                name = "__test__%s__%s__%d" % (case.name, k, i)
                                define_cbs[v] = name
                                f.writeln(
                                    "intmax_t %s("
                ...
                if suite.if_ is not None or case.if_ is not None:
                    f.writeln("bool __test__%s__filter(void) {" % (case.name))
                    f.writeln(
                        4 * " "
                        + "return %s;"
                        % " && ".join(
                ...
                f.writeln(
                    "void __test__%s__run("
                    "__attribute__((unused)) struct lfs_config *cfg) {" % (case.name)
                )
                f.writeln(4 * " " + "// test case %s" % case.name)
                if case.code_lineno is not None:
                    f.writeln(
                        4 * " " + '#line %d "%s"' % (case.code_lineno, suite.path)
                    )
                f.write(case.code)
                if case.code_lineno is not None:
                    f.writeln(
                        4 * " " + '#line %d "%s"' % (f.lineno + 1, args["output"])
                    )
                f.writeln("}")
                f.writeln()
            ...
            if not args.get("source"):
                if suite.code is not None:
                    if suite.code_lineno is not None:
                        f.writeln('#line %d "%s"' % (suite.code_lineno, suite.path))
                    f.write(suite.code)
                    if suite.code_lineno is not None:
                        f.writeln('#line %d "%s"' % (f.lineno + 1, args["output"]))
                    f.writeln()
                ...
                if suite.defines:
                    for i, define in enumerate(sorted(suite.defines)):
                        ...
                        f.writeln(
                            "#define %-24s " "TEST_DEFINE(%s)" % (define, define + "_i")
                        )
                        f.writeln("#endif")
                    f.writeln()
                ...
                for case in suite.cases:
                    if case.in_ is None:
                        ...
                        if suite.if_ is not None or case.if_ is not None:
                            f.writeln(
                                "extern bool __test__%s__filter(" "void);" % (case.name)
                            )
                        f.writeln(
                            "extern void __test__%s__run("
                            "struct lfs_config *cfg);" % (case.name)
                        )
                        f.writeln()
                ...
                f.writeln('__attribute__((section("_test_suites"), ' "aligned(1)))")
                f.writeln("const struct test_suite __test__%s__suite = {" % suite.name)
                ...
                f.writeln(
                    ...
                    % (
                        " | ".join(
                            filter(
                                None, ["TEST_REENTRANT" if suite.reentrant else None]
                            )
                        )
                        or 0
                    )
                ...
                if suite.defines:
                    ...
                    f.writeln(
                        4 * " " + ".define_names = (const char *const["
                ...
                for case in suite.cases:
                    ...
                    f.writeln(
                        ...
                        % (
                            " | ".join(
                                filter(
                                    None, ["TEST_REENTRANT" if case.reentrant else None]
                                )
                            )
                            or 0
                        )
                    )
                    f.writeln(12 * " " + ".permutations = %d," % len(case.permutations))
                    if case.defines:
                        f.writeln(
                            12 * " " + ".defines "
                            "= (const test_define_t*)__test__%s__defines," % (case.name)
                        )
                    if suite.if_ is not None or case.if_ is not None:
                        f.writeln(
                            12 * " " + ".filter = __test__%s__filter," % (case.name)
                        )
                    f.writeln(12 * " " + ".run = __test__%s__run," % (case.name))
                    f.writeln(8 * " " + "},")
                f.writeln(4 * " " + "},")
                ...
                for suite in suites:
                    for case in suite.cases:
                        if case.in_ is not None and os.path.normpath(
                            case.in_
                            ...
                            if suite.defines:
                                for i, define in enumerate(sorted(suite.defines)):
                                    ...
                                    f.writeln(
                                        "#define " "__TEST__%s__NEEDS_UNDEF" % (define)
                                    )
                                    f.writeln("#endif")
                                f.writeln()
                            ...
                            if suite.defines:
                                for define in sorted(suite.defines):
                                    f.writeln("#ifdef __TEST__%s__NEEDS_UNDEF" % define)
                                    f.writeln("#undef __TEST__%s__NEEDS_UNDEF" % define)
...
def find_runner(runner, **args):
    ...
    # run under perf?
    if args.get("perf"):
        cmd[:0] = args["perf_script"] + list(
            filter(
                None,
                [
                    "-R",
                    "--perf-freq=%s" % args["perf_freq"]
                    if args.get("perf_freq")
                    else None,
                    "--perf-period=%s" % args["perf_period"]
                    if args.get("perf_period")
                    else None,
                    "--perf-events=%s" % args["perf_events"]
                    if args.get("perf_events")
                    else None,
                    "--perf-path=%s" % args["perf_path"]
                    if args.get("perf_path")
                    else None,
                    "-o%s" % args["perf"],
                ],
            )
...
def list_(runner, test_ids=[], **args):
    ...
    if args.get("summary"):
        cmd.append("--summary")
    if args.get("list_suites"):
        cmd.append("--list-suites")
    if args.get("list_cases"):
        cmd.append("--list-cases")
    if args.get("list_suite_paths"):
        cmd.append("--list-suite-paths")
    if args.get("list_case_paths"):
        cmd.append("--list-case-paths")
    if args.get("list_defines"):
        cmd.append("--list-defines")
    ...
    if args.get("list_geometries"):
        cmd.append("--list-geometries")
    if args.get("list_powerlosses"):
        cmd.append("--list-powerlosses")
...
def find_perms(runner_, ids=[], **args):
    ...
    pattern = re.compile(r"^(?P<case>[^\s]+)" r"\s+(?P<path>[^:]+):(?P<lineno>\d+)")
    # skip the first line
    for line in it.islice(proc.stdout, 1, None):
        m = pattern.match(line)
        if m:
...
def run_stage(name, runner_, ids, stdout_, trace_, output_, **args):
    ...
    def run_runner(runner_, ids=[]):
                ...
                if m:
                    ...
                    if op == "running":
                        ...
                        if output_:
                            ...
                            defines = find_defines(runner_, m.group("id"), **args)
                            output_.writerow(
                                {
                                    "suite": suite,
                                    "case": case,
        ...
        proc.wait()
        if proc.returncode != 0:
            raise TestFailure(last_id, proc.returncode, list(last_stdout), last_assert)
        ...
        while start < total_perms:
            ...
            try:
                ...
                if output_:
                    ...
                    output_.writerow(
                        {"suite": suite, "case": case, "test_passed": "0/1", **defines}
                    )
    ...
    if "jobs" in args:
        for job in range(args["jobs"]):
            runners.append(
                th.Thread(
                    target=run_job, args=(runner_, ids, job, args["jobs"]), daemon=True
                )
            )
    else:
        runners.append(
            th.Thread(target=run_job, args=(runner_, ids, None, None), daemon=True)
        )
                    ...
                    ", ".join(
                        filter(
                            ...
                            [
                                ...
                                % (
                                    sum(
                                        passed_suite_perms[k] == v
                                        for k, v in expected_suite_perms.items()
                                    ),
                                    len(expected_suite_perms),
                                )
                                if (
                                    not args.get("by_suites")
                                    and not args.get("by_cases")
                                )
                                else None,
                                "%d/%d cases"
                                % (
                                    sum(
                                        passed_case_perms[k] == v
    ...
    return (expected_perms, passed_perms, powerlosses, failures, killed)
...
def run(runner, test_ids=[], **args):
    ...
    (_, expected_suite_perms, expected_case_perms, expected_perms, total_perms) = (
        find_perms(runner_, test_ids, **args)
    )
    print(
    ...
    for by in (
        test_ids
        if test_ids
        else expected_case_perms.keys()
        if args.get("by_cases")
        else expected_suite_perms.keys()
    ...
    print(
        "%sdone:%s %s"
        % (
            ("\x1b[32m" if not failures else "\x1b[31m") if args["color"] else "",
            "\x1b[m" if args["color"] else "",
            ", ".join(
                filter(
                    None,
    ...
    if failures and (
        ...
        if args.get("gdb_main"):
            ...
            powerlosses = (
                sum(1 for _ in re.finditer("[0-9a-f]", failure.id.split(":", 2)[-1]))
                if failure.id.count(":") >= 2
                else 0
            )
            path, lineno = find_path(runner_, failure.id, **args)
            cmd[:0] = args["gdb_path"] + [
                ...
                "break %s:%d" % (path, lineno),
                "-ex",
                "ignore 1 %d" % max(powerlosses - 1, 0),
                "-ex",
                "run",
                "--args",
            ]
            ...
            cmd[:0] = args["gdb_path"] + ["-ex", "run", "--args"]
...
if __name__ == "__main__":
    ...
    test_parser.add_argument("test_ids", nargs="*", help="Description of tests to run.")
    test_parser.add_argument(
        "-Y", "--summary", action="store_true", help="Show quick summary."
    )
    test_parser.add_argument(
        "-l", "--list-suites", action="store_true", help="List test suites."
    )
    test_parser.add_argument(
        "-L", "--list-cases", action="store_true", help="List test cases."
    )
    test_parser.add_argument(
        "--list-suite-paths",
        action="store_true",
    ...
    test_parser.add_argument(
        "-D", "--define", action="append", help="Override a test define."
    )
    test_parser.add_argument(
        "-G", "--geometry", help="Comma-separated list of disk geometries to test."
    )
    test_parser.add_argument(
        "-P",
        "--powerloss",
    ...
    test_parser.add_argument(
        "-d", "--disk", help="Direct block device operations to this file."
    )
    test_parser.add_argument("-t", "--trace", help="Direct trace output to this file.")
    test_parser.add_argument(
        "--trace-backtrace",
        action="store_true",
    ...
    test_parser.add_argument(
        "--trace-period", help="Sample trace output at this period in cycles."
    )
    test_parser.add_argument(
        "--trace-freq", help="Sample trace output at this frequency in hz."
    )
    test_parser.add_argument(
        "-O",
        "--stdout",
    ...
    test_parser.add_argument("-o", "--output", help="CSV file to store results.")
    test_parser.add_argument("--read-sleep", help="Artificial read delay in seconds.")
    test_parser.add_argument("--prog-sleep", help="Artificial prog delay in seconds.")
    test_parser.add_argument("--erase-sleep", help="Artificial erase delay in seconds.")
    test_parser.add_argument(
        "-j",
        "--jobs",
        nargs="?",
    ...
    test_parser.add_argument(
        "-k", "--keep-going", action="store_true", help="Don't stop on first error."
    )
    test_parser.add_argument(
        "-i",
        "--isolate",
    ...
    test_parser.add_argument(
        "-b", "--by-suites", action="store_true", help="Step through tests by suite."
    )
    test_parser.add_argument(
        "-B", "--by-cases", action="store_true", help="Step through tests by case."
    )
    test_parser.add_argument(
        "--context",
        type=lambda x: int(x, 0),
        default=5,
        help="Show this many lines of stdout on test failure. " "Defaults to 5.",
    )
    test_parser.add_argument(
        "--gdb", action="store_true", help="Drop into gdb on test failure."
    )
    test_parser.add_argument(
        "--gdb-case",
        action="store_true",
    ...
    test_parser.add_argument(
        "--gdb-main",
        action="store_true",
        help="Drop into gdb on test failure but stop at the beginning " "of main.",
    )
    test_parser.add_argument(
        "--gdb-pl",
        type=lambda x: int(x, 0),
    ...
    test_parser.add_argument(
        "--exec", type=lambda e: e.split(), help="Run under another executable."
    )
    test_parser.add_argument(
        "--valgrind",
        action="store_true",
        help="Run under Valgrind to find memory errors. Implicitly sets " "--isolate.",
    )
    test_parser.add_argument(
        "--valgrind-path",
        type=lambda x: x.split(),
    ...
    test_parser.add_argument(
        "--perf-freq",
        help="perf sampling frequency. This is passed directly to the perf " "script.",
    )
    test_parser.add_argument(
        "--perf-period",
        help="perf sampling period. This is passed directly to the perf " "script.",
    )
    test_parser.add_argument(
        "--perf-events",
        help="perf events to record. This is passed directly to the perf " "script.",
    )
    test_parser.add_argument(
        "--perf-script",
        type=lambda x: x.split(),
    ...
    comp_parser.add_argument("-o", "--output", help="Output file.")
