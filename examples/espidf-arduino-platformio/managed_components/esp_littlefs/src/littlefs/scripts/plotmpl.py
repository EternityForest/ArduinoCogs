        ...
        for x_ in x if x is not None else [None]:
            for y_ in y:
                ...
                datasets[ks_ + (k_x, k_y)] = dataset(
                    ...
                    x_,
                    y_,
                    [(by_, {k_}) for by_, k_ in zip(by, ks_)] if by is not None else [],
                )
...
class Grid:
    ...
    def merge(self, other, dir):
        if dir in ["above", "below"]:
            ...
            other_i = 0
            self_xweight = (
                self_xweights[self_i] if self_i < len(self_xweights) else m.inf
            )
            other_xweight = (
                other_xweights[other_i] if other_i < len(other_xweights) else m.inf
            )
            while self_i < len(self_xweights) and other_i < len(other_xweights):
                if other_xweight - self_xweight > 0.0000001:
                    ...
                    other_i += 1
                    other_xweight = (
                        other_xweights[other_i]
                        if other_i < len(other_xweights)
                        else m.inf
                    )
                else:
                    new_xweights.append(self_xweight)
            ...
            if dir == "above":
                ...
                self.map = self_map | {
                    (x, y + len(self_yweights)): s for (x, y), s in other_map.items()
                }
            else:
                for s in self.subplots:
                ...
                self.map = other_map | {
                    (x, y + len(other.yweights)): s for (x, y), s in self_map.items()
                }
        ...
        if dir in ["right", "left"]:
            ...
            other_i = 0
            self_yweight = (
                self_yweights[self_i] if self_i < len(self_yweights) else m.inf
            )
            other_yweight = (
                other_yweights[other_i] if other_i < len(other_yweights) else m.inf
            )
            while self_i < len(self_yweights) and other_i < len(other_yweights):
                if other_yweight - self_yweight > 0.0000001:
                    ...
                    other_i += 1
                    other_yweight = (
                        other_yweights[other_i]
                        if other_i < len(other_yweights)
                        else m.inf
                    )
                else:
                    new_yweights.append(self_yweight)
            ...
            if dir == "right":
                ...
                self.map = self_map | {
                    (x + len(self_xweights), y): s for (x, y), s in other_map.items()
                }
            else:
                for s in self.subplots:
                ...
                self.yweights = new_yweights
                self.map = other_map | {
                    (x + len(other.xweights), y): s for (x, y), s in self_map.items()
                }
    ...
    @classmethod
    def fromargs(cls, width=1.0, height=1.0, *, subplots=[], **args):
        grid = cls(Subplot(**args))

        for dir, subargs in subplots:
            subgrid = cls.fromargs(
                width=subargs.pop("width", 0.5 if dir in ["right", "left"] else width),
                height=subargs.pop(
                    "height", 0.5 if dir in ["above", "below"] else height
                ),
                **subargs,
            )
...
def main(
    ...
    **args,
    ...
    plt.rc("figure", titlesize="medium", labelsize="small")
    plt.rc("axes", titlesize="small", labelsize="small", labelcolor=font_color_)
    if not ggplot:
    ...
    renames = list(
        it.chain.from_iterable(
            ((k, v) for v in vs) for k, vs in it.chain(by or [], x or [], y or [])
        )
    )
    if by is not None:
        by = [k for k, _ in by]
    ...
    dataformats_ = {
        name: formats_[i % len(formats_)] for i, name in enumerate(datasets_.keys())
    }
    datacolors_ = {
        name: colors_[i % len(colors_)] for i, name in enumerate(datasets_.keys())
    }

    # create a grid of subplots
    grid = Grid.fromargs(subplots=subplots + subplot.pop("subplots", []), **subplot)
    ...
    gs = fig.add_gridspec(
        grid.height + (1 if legend_above else 0) + (1 if legend_below else 0),
        grid.width + (1 if legend_right else 0),
    ...
    for s in grid:
        s.ax = fig.add_subplot(
            gs[
                grid.height - (s.y + s.yspan) + (1 if legend_above else 0) : grid.height
                - s.y
    ...
    for s in grid:
        ...
        if x2_:
            ax.xaxis.set_major_formatter(
                lambda x, pos: si2(x) + (xunits_ if xunits_ else "")
            )
            ...
            ax.xaxis.set_major_formatter(
                lambda x, pos: si(x) + (xunits_ if xunits_ else "")
            )
            if xticklabels_ is not None:
                ax.xaxis.set_ticklabels(xticklabels_)
            if xticks_ is None:
        ...
        if y2_:
            ax.yaxis.set_major_formatter(
                lambda x, pos: si2(x) + (yunits_ if yunits_ else "")
            )
            ...
            ax.yaxis.set_major_formatter(
                lambda x, pos: si(x) + (yunits_ if yunits_ else "")
            )
            if yticklabels_ is not None:
                ax.yaxis.set_ticklabels(yticklabels_)
            if yticks_ is None:
    ...
    if legend_above:
        ...
        for ncol in reversed(range(1, len(labels) + 1)):
            ...
            if legend_.get_window_extent().width <= ax.get_window_extent().width:
                break
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Plot CSV files with matplotlib.", allow_abbrev=False
    )
    parser.add_argument("csv_paths", nargs="*", help="Input *.csv files.")
    output_rule = parser.add_argument(
        "-o", "--output", required=True, help="Output *.svg/*.png file."
    )
    parser.add_argument(
        "--svg",
        action="store_true",
    ...
    parser.add_argument("-q", "--quiet", action="store_true", help="Don't print info.")
    parser.add_argument(
        "-b",
        "--by",
        action="append",
        type=lambda x: (lambda k, v=None: (k, v.split(",") if v is not None else ()))(
            *x.split("=", 1)
        ),
    ...
    parser.add_argument(
        "-.", "--points", action="store_true", help="Only draw data points."
    )
    parser.add_argument(
        "-!",
        "--points-and-lines",
    ...
    parser.add_argument(
        "-X",
        "--xlim",
        type=lambda x: tuple(dat(x) if x.strip() else None for x in x.split(",")),
    ...
    parser.add_argument("--xlog", action="store_true", help="Use a logarithmic x-axis.")
    parser.add_argument("--ylog", action="store_true", help="Use a logarithmic y-axis.")
    parser.add_argument(
        "--x2", action="store_true", help="Use base-2 prefixes for the x-axis."
    )
    parser.add_argument(
        "--y2", action="store_true", help="Use base-2 prefixes for the y-axis."
    )
    parser.add_argument(
        "--xticks",
        type=lambda x: int(x, 0) if "," not in x else [dat(x) for x in x.split(",")],
    ...
    parser.add_argument("--xunits", help="Units for the x-axis.")
    parser.add_argument("--yunits", help="Units for the y-axis.")
    parser.add_argument("--xlabel", help="Add a label to the x-axis.")
    parser.add_argument("--ylabel", help="Add a label to the y-axis.")
    parser.add_argument(
        "--xticklabels",
        type=lambda x: [x.strip() for x in x.split(",")] if x.strip() else [],
    ...
    parser.add_argument("-t", "--title", help="Add a title.")
    parser.add_argument(
        "-l", "--legend-right", action="store_true", help="Place a legend to the right."
    )
    parser.add_argument(
        "--legend-above", action="store_true", help="Place a legend above."
    )
    parser.add_argument(
        "--legend-below", action="store_true", help="Place a legend below."
    )
    parser.add_argument("--dark", action="store_true", help="Use the dark style.")
    parser.add_argument("--ggplot", action="store_true", help="Use the ggplot style.")
    parser.add_argument("--xkcd", action="store_true", help="Use the xkcd style.")
    parser.add_argument(
        "--github", action="store_true", help="Use the ggplot style with GitHub colors."
    )
    parser.add_argument(
        "--font",
        type=lambda x: [x.strip() for x in x.split(",")],
    ...
    parser.add_argument(
        "--font-size", help="Font size for matplotlib. Defaults to %r." % FONT_SIZE
    )
    parser.add_argument(
        "--font-color", help="Color for the font and other line elements."
    )
    parser.add_argument("--foreground", help="Foreground color to use.")
    parser.add_argument("--background", help="Background color to use.")
    ...
    class AppendSubplot(argparse.Action):
        ...
        def parse(value):
            ...
            next(
                a for a in subparser._actions if "--output" in a.option_strings
            ).required = False
            next(
                a for a in subparser._actions if "--width" in a.option_strings
            ).type = float
            next(
                a for a in subparser._actions if "--height" in a.option_strings
            ).type = float
        ...
        def __call__(self, parser, namespace, value, option):
            ...
            namespace.subplots.append(
                (option.split("-")[-1], self.__class__.parse(value))
            )
    ...
    def dictify(ns):
        if hasattr(ns, "subplots"):
            ns.subplots = [
                (dir, dictify(subplot_ns)) for dir, subplot_ns in ns.subplots
            ]
