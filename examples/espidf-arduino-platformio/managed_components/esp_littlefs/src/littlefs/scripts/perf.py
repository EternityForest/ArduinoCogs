    ...
    with tempfile.NamedTemporaryFile("rb") as f:
        ...
        perf = perf_path + list(
            filter(
                ...
                [
                    "record",
                    "-F%s" % perf_freq
                    if perf_freq is not None and perf_period is None
                    else None,
                    "-c%s" % perf_period if perf_period is not None else None,
                    "-B",
                    "-g",
                    "--all-user",
                    "-e%s" % perf_events,
        ...
        with os.fdopen(z, "r+b") as z:
            with zipfile.ZipFile(
                z, "a", compression=zipfile.ZIP_DEFLATED, compresslevel=1
            ) as z:
...
def collect_syms_and_lines(obj_path, *, objdump_path=None, **args):
    ...
    for line in proc.stdout:
        ...
        if m:
            if m.group("no") and not m.group("dir"):
                ...
                dir = int(m.group("dir"))
                if dir in dirs:
                    files[int(m.group("no"))] = os.path.join(dirs[dir], m.group("path"))
                else:
                ...
                if m.group("op_special") or m.group("op_copy") or m.group("op_end"):
...
def collect_decompressed(
    ...
    **args,
    ...
    last_period = 0
    last_stack = []
    deltas = co.defaultdict(dict)
    syms_ = co.defaultdict(dict)
    at_cache = {}
    results = {}
    ...
    for line in proc.stdout:
        ...
        if not line.startswith("\t"):
            ...
            if m:
                ...
                syms, sym_at, lines, line_at = collect_syms_and_lines(dso, **args)
                ...
                if sym in syms:
                    ...
                    delta, _ = min(deltas[dso].items(), key=lambda x: (x[1], x[0]))
                    addr = addr_ + delta
                    ...
                    if (dso, addr) in at_cache:
                        ...
                        if sources is not None:
                            ...
                            if (
                                ...
                                and not os.path.commonpath(
                                    [os.getcwd(), os.path.abspath(file)]
                                )
                                == os.getcwd()
                            ):
                                at_cache[(dso, addr)] = None
                        ...
                        # simplify path
                        if (
                            os.path.commonpath([os.getcwd(), os.path.abspath(file)])
                            == os.getcwd()
                        ):
...
def collect(perf_paths, *, jobs=None, **args):
    ...
    if jobs is not None:
        ...
        with mp.Pool(jobs) as p:
            for results_ in p.imap_unordered(
                starapply, ((collect_job, (path, i), args) for path, i in records)
            ):
...
def fold(Result, results, *, by=None, defines=None, **_):
    if by is None:
        by = Result._by
    ...
    for r in folded:
        folded_.append(
            r._replace(children=fold(Result, r.children, by=by, defines=defines))
        )
    folded = folded_
...
def table(
    ...
    **_,
    ...
    # organize by name
    table = {",".join(str(getattr(r, k) or "") for k in by): r for r in results}
    diff_table = {
        ",".join(str(getattr(r, k) or "") for k in by): r for r in diff_results or []
    }
    ...
    if diff_results is not None:
        names.sort(
            key=lambda n: tuple(
                types[k].ratio(
                    getattr(table.get(n), k, None), getattr(diff_table.get(n), k, None)
                )
                for k in fields
            ),
            reverse=True,
        ...
        for k, reverse in reversed(sort):
            names.sort(
                key=lambda n: tuple(
                    ...
                    for k in ([k] if k else [k for k in Result._sort if k in fields])
                ),
    ...
    header.append(
        ...
        % (
            ...
            if diff_results is not None and not percent
            else "",
        )
        if not summary
        else ""
    ...
    def table_entry(name, r, diff_r=None, ratios=[]):
        ...
        if diff_results is None:
            ...
            for k in fields:
                entry.append(
                    types[k].diff_diff(getattr(r, k, None), getattr(diff_r, k, None))
                )
        if diff_results is None:
            entry.append("")
        elif percent:
    ...
    if not summary:
        for name in names:
            ...
            if diff_results is None:
                ...
                ratios = [
                    types[k].ratio(getattr(r, k, None), getattr(diff_r, k, None))
                    for k in fields
                ]
    ...
    widths = [
        ...
        for w, i in zip(it.chain([23], it.repeat(7)), range(len(lines[0]) - 1))
    ]
    ...
    print(
        ...
        % (
            widths[0],
            lines[0][0],
            " ".join("%*s" % (w, x) for w, x in zip(widths[1:], lines[0][1:-1])),
            lines[0][-1],
        )
    )
    ...
    if not summary:
        ...
        def recurse(results_, depth_, prefixes=("", "", "", "")):
            ...
            table_ = {
                ",".join(str(getattr(r, k) or "") for k in by): r for r in results_
            }
                ...
                for k, reverse in reversed(sort):
                    names_.sort(
                        key=lambda n: tuple(
                            ...
                            for k in (
                                [k] if k else [k for k in Result._sort if k in fields]
                            )
                        ),
                        reverse=reverse ^ (not k or k in Result._fields),
                    )
            ...
            for i, name in enumerate(names_):
                ...
                print(
                    ...
                    % (
                        prefixes[0 + is_last],
                        widths[0]
                        - (len(prefixes[0 + is_last]) if not m.isinf(depth) else 0),
                        name,
                        " ".join(
                            "%*s" % (w, x)
                            for w, x in zip(widths[1:], table_entry(name, r)[1:])
                        ),
                    )
                )
        ...
        for name, line in zip(names, lines[1:-1]):
            print(
                ...
                % (
                    widths[0],
                    line[0],
                    " ".join("%*s" % (w, x) for w, x in zip(widths[1:], line[1:-1])),
                    line[-1],
                )
            )
    ...
    print(
        ...
        % (
            widths[0],
            lines[-1][0],
            " ".join("%*s" % (w, x) for w, x in zip(widths[1:], lines[-1][1:-1])),
            lines[-1][-1],
        )
    )
            ...
            for line, r in sorted(table.items()):
                if float(getattr(r, tk)) / max_ >= t0:
                    if last is not None and line - last.stop <= args["context"]:
                        last = range(last.start, line + 1 + args["context"])
                    else:
                        if last is not None:
                            spans.append((last, func))
                        last = range(line - args["context"], line + 1 + args["context"])
                        func = r.function
            if last is not None:
                spans.append((last, func))
    ...
    **args,
    ...
    if not args.get("use", None):
        ...
        with openio(args["use"]) as f:
            ...
            for r in reader:
                if not any(
                    "perf_" + k in r and r["perf_" + k].strip()
    ...
    if args.get("output"):
        with openio(args["output"], "w") as f:
            writer = csv.DictWriter(
                ...
                + [
                    "perf_" + k
                    for k in (fields if fields is not None else PerfResult._fields)
                ],
            )
            writer.writeheader()
            for r in results:
    ...
    if not args.get("quiet"):
        if args.get("annotate") or args.get("threshold"):
            # annotate sources
            annotate(PerfResult, results, branches=branches, caches=caches, **args)
        else:
            # print table
            table(
                PerfResult,
...
if __name__ == "__main__":
    ...
    parser.add_argument("perf_paths", nargs=nargs, help="Input *.perf files.")
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
    ...
    parser.add_argument("-o", "--output", help="Specify CSV file to store results.")
    parser.add_argument("-u", "--use", help="Don't parse anything, use this CSV file.")
    parser.add_argument("-d", "--diff", help="Specify CSV file to diff against.")
    parser.add_argument(
        "-a",
        "--all",
        action="store_true",
    ...
    parser.add_argument(
        "-s", "--sort", nargs="?", action=AppendSort, help="Sort by this field."
    )
    parser.add_argument(
        "-S",
        "--reverse-sort",
    ...
    parser.add_argument(
        "-Y", "--summary", action="store_true", help="Only show the total."
    )
    parser.add_argument(
        "-F",
        "--source",
    ...
    parser.add_argument(
        "--branches", action="store_true", help="Show branches and branch misses."
    )
    parser.add_argument(
        "--caches", action="store_true", help="Show cache accesses and cache misses."
    )
    parser.add_argument(
        "-P",
        "--propagate",
    ...
    record_parser.add_argument("command", nargs=nargs, help="Command to run.")
    ...
    record_parser.add_argument(
        "--perf-period", help="perf sampling period. This is passed directly to perf."
    )
    record_parser.add_argument(
        "--perf-events",
