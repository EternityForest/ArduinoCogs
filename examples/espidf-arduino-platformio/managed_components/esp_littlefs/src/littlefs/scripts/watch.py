    ...
    try:
        while True:
            ...
            try:
                ...
                if lines:
                    h = lines
                fcntl.ioctl(spty, termios.TIOCSWINSZ, struct.pack("HHHH", h, w, 0, 0))

                proc = sp.Popen(command, stdout=spty, stderr=spty, close_fds=False)
                os.close(spty)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-e", "--exit-on-error", action="store_true", help="Exit on error."
    )
    sys.exit(
        main(**{k: v for k, v in vars(parser.parse_args()).items() if v is not None})
    )
