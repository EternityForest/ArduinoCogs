...
class Frac(co.namedtuple("Frac", "a,b")):
    ...
    def table(self):
        ...
        return "%11s %7s" % (
            self,
            "∞%" if t == +m.inf else "-∞%" if t == -m.inf else "%.1f%%" % (100 * t),
        )
    ...
    def diff_diff(self, other):
        ...
        return "%11s" % (
            "%s/%s" % (new_a.diff_diff(old_a).strip(), new_b.diff_diff(old_b).strip())
        )
...
class CovResult(
    co.namedtuple(
        "CovResult",
        ["file", "function", "line", "calls", "hits", "funcs", "lines", "branches"],
    )
):
    _by = ["file", "function", "line"]
    _fields = ["calls", "hits", "funcs", "lines", "branches"]
    _sort = ["funcs", "lines", "branches", "hits", "calls"]
    _types = {"calls": Int, "hits": Int, "funcs": Frac, "lines": Frac, "branches": Frac}

    __slots__ = ()

    def __new__(
        cls, file="", function="", line=0, calls=0, hits=0, funcs=0, lines=0, branches=0
    ):
...
def collect(gcda_paths, *, gcov_path=GCOV_PATH, sources=None, everything=False, **args):
    results = []
    for path in gcda_paths:
        ...
        for file in data["files"]:
            ...
            if sources is not None:
                if not any(
                    os.path.abspath(file["file"]) == os.path.abspath(s) for s in sources
                ):
                    continue
            else:
                # default to only cwd
                if (
                    ...
                    and not os.path.commonpath(
                        [os.getcwd(), os.path.abspath(file["file"])]
                    )
                    == os.getcwd()
                ):
                    continue
            ...
            # simplify path
            if (
                os.path.commonpath([os.getcwd(), os.path.abspath(file["file"])])
                == os.getcwd()
            ):
...
def annotate(Result, results, *, annotate=False, lines=False, branches=False, **args):
    ...
    for path in co.OrderedDict.fromkeys(r.file for r in results).keys():
        ...
        if not annotate:
            ...
            for line, r in sorted(table.items()):
                if (lines and int(r.hits) == 0) or (
                    branches and r.branches.a < r.branches.b
                ):
            ...
            for i, line in enumerate(f):
                ...
                if i + 1 in table:
                    ...
                    line = "%-*s // %s hits%s" % (
                        ...
                        line,
                        r.hits,
                        ", %s branches" % (r.branches,) if int(r.branches.b) else "",
                    )
...
def main(
    gcda_paths, *, by=None, fields=None, defines=None, sort=None, hits=False, **args
):
    ...
    if not args.get("use", None):
        ...
        with openio(args["use"]) as f:
            ...
            for r in reader:
                if not any(
                    "cov_" + k in r and r["cov_" + k].strip() for k in CovResult._fields
                ):
                    continue
                try:
                    results.append(
                        CovResult(
                            **{
                                k: r[k]
                                for k in CovResult._by
                                if k in r and r[k].strip()
                            },
                            **{
                                k: r["cov_" + k]
                                for k in CovResult._fields
                                if "cov_" + k in r and r["cov_" + k].strip()
                            },
                        )
                    )
                except TypeError:
    ...
    if args.get("output"):
        with openio(args["output"], "w") as f:
            writer = csv.DictWriter(
                ...
                + [
                    "cov_" + k
                    for k in (fields if fields is not None else CovResult._fields)
                ],
            )
            writer.writeheader()
            for r in results:
    ...
    if args.get("diff"):
        ...
        try:
            with openio(args["diff"]) as f:
                ...
                for r in reader:
                    if not any(
                        "cov_" + k in r and r["cov_" + k].strip()
                        for k in CovResult._fields
                    ):
                        continue
                    try:
    ...
    if not args.get("quiet"):
        if args.get("annotate") or args.get("lines") or args.get("branches"):
            # annotate sources
    ...
    if args.get("error_on_lines") and any(r.lines.a < r.lines.b for r in results):
        sys.exit(2)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Find coverage info after running tests.", allow_abbrev=False
    )
    parser.add_argument("gcda_paths", nargs="*", help="Input *.gcda files.")
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
    ...
    parser.add_argument(
        "--hits", action="store_true", help="Show total hits instead of coverage."
    )
    parser.add_argument(
        "-A",
        "--annotate",
    ...
    parser.add_argument(
        "-L", "--lines", action="store_true", help="Show uncovered lines."
    )
    parser.add_argument(
        "-B", "--branches", action="store_true", help="Show uncovered branches."
    )
    parser.add_argument(
        "-c",
        "--context",
