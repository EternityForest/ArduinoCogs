...
from readmdir import MetadataPair, Tag
...
def main(args):
    ...
    with open(args.disk, "rb") as f:
        ...
        while True:
            ...
            for block in tail:
                ...
                data.append(f.read(args.block_size).ljust(args.block_size, b"\xff"))
            ...
            # have superblock?
            try:
                nsuperblock = mdir[Tag(0x7FF, 0x3FF, 0), Tag("superblock", 0, 0)]
            ...
            try:
                ...
                gstate = bytes(
                    (a or 0) ^ (b or 0) for a, b in it.zip_longest(gstate, ngstate.data)
                )
            except KeyError:
                pass
    ...
    if superblock:
        version = tuple(
            reversed(struct.unpack("<HH", superblock[1].data[0:4].ljust(4, b"\xff")))
        )
    print(
        "%-47s%s"
        % (
    ...
    if tag.type:
        print("  move dir {%#x, %#x} id %d" % (blocks[0], blocks[1], tag.id))

    # print mdir info
    for i, dir in enumerate(dirs):
        print(
            "dir %s"
            % (json.dumps(dir[0].path) if hasattr(dir[0], "path") else "(orphan)")
        )

        for j, mdir in enumerate(dir):
            print(
                "mdir {%#x, %#x} rev %d (was %d)%s%s"
                % (
                    mdir.blocks[0],
                    mdir.blocks[1],
                    mdir.rev,
    ...
    for mdir in corrupted:
        ...
        print("*** corrupted mdir {%#x, %#x}! ***" % (mdir.blocks[0], mdir.blocks[1]))
