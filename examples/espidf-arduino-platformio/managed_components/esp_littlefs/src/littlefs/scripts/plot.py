...
class Plot:
    def __init__(
        ...
        *,
        ...
        self.height = 4 * height if braille else 2 * height if dots else height
    ...
    def scale(self, x, y):
        ...
        try:
            if self.xlog:
                ...
                x = int(
                    self.width * ((x - self.xlim[0]) / (self.xlim[1] - self.xlim[0]))
                )
            if self.ylog:
                y = int(
                    self.height
                ...
                y = int(
                    self.height * ((y - self.ylim[0]) / (self.ylim[1] - self.ylim[0]))
                )
    ...
    def point(self, x, y, *, color=COLORS[0], char=True):
        # scale
        x, y = self.scale(x, y)
    ...
    def line(self, x1, y1, x2, y2, *, color=COLORS[0], char=True):
        # scale
    ...
    def plot(self, coords, *, color=COLORS[0], char=True, line_char=True):
        # draw lines
        if line_char:
            for (x1, y1), (x2, y2) in zip(coords, coords[1:]):
                if y1 is not None and y2 is not None:
                    self.line(x1, y1, x2, y2, color=color, char=line_char)

        # draw points
        if char and (not line_char or char is not True):
            for x, y in coords:
                if y is not None:
                    self.point(x, y, color=color, char=char)

    def draw(self, row, *, color=False):
        # scale if needed
        if self.braille:
            xscale, yscale = 2, 4
        elif self.dots:
...
def main(
    ...
    **args,
    ...
    for s in grid:
        ...
        subtitle = escape(subtitle).splitlines() if subtitle is not None else []
        xsublabel = escape(xsublabel).splitlines() if xsublabel is not None else []
        ysublabel = escape(ysublabel).splitlines() if ysublabel is not None else []
    ...
    for s in grid:
        s.xmargin = (
            len(s.ylabel)
            + (1 if s.ylabel else 0)  # fit ysublabel
    ...
    def draw(f):
        ...
        datachars_ = {
            name: chars_[i % len(chars_)] for i, name in enumerate(datasets_.keys())
        }
        dataline_chars_ = {
        ...
        if height is None:
            ...
            height_ = shutil.get_terminal_size((None, 17 + len(title) + len(xlabel)))[1]
        ...
        for line in title:
            f.writeln(
                "%*s%s" % (sum(xmargin[:2]), "", line.center(width_ - xmargin[1]))
            )
            ...
            for x_ in range(grid.width):
                ...
                if subrow < s.ymargin[-1]:
                    ...
                    if subrow < len(s.title):
                        f.write(
                            "%*s%s"
                            % (sum(s.xmargin[:2]), "", s.title[subrow].center(s.width))
                        )
                    else:
                        f.write("%*s%*s" % (sum(s.xmargin[:2]), "", s.width, ""))
                # draw plot?
                    ...
                    f.write(
                        ...
                        % (
                            ...
                            "%s "
                            % "".join(
                                line.center(s.height)[subrow] for line in s.ylabel
                            )
                            if s.ylabel
                            else "",
                        )
                ...
                # footer
                    ...
                    # draw xsublabel?
                    elif subrow < s.ymargin[1] or subrow - s.ymargin[1] >= len(
                        s.xlabel
                    ):
                        f.write("%*s%*s" % (sum(s.xmargin[:2]), "", s.width, ""))
                    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Plot CSV files in terminal.", allow_abbrev=False
    )
    ...
    parser.add_argument("--chars", help="Characters to use for points.")
    parser.add_argument("--line-chars", help="Characters to use for lines.")
    parser.add_argument(
        "-W",
        "--width",
        nargs="?",
    ...
    parser.add_argument(
        "-z", "--cat", action="store_true", help="Pipe directly to stdout."
    )
    parser.add_argument(
        "-k",
        "--keep-open",
