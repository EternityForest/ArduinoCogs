...
# code size results
class CodeResult(co.namedtuple("CodeResult", ["file", "function", "size"])):
    _by = ["file", "function"]
    _fields = ["size"]
    _sort = ["size"]
    _types = {"size": Int}
    ...
    def __add__(self, other):
        return CodeResult(self.file, self.function, self.size + other.size)
        ...
        for line in proc.stdout:
            ...
            if m:
                ...
                if not everything and func.startswith("__"):
                    continue
                results_.append(CodeResult(file, func, int(m.group("size"), 16)))
        proc.wait()
...
def main(obj_paths, *, by=None, fields=None, defines=None, sort=None, **args):
    ...
    if not args.get("use", None):
        ...
        with openio(args["use"]) as f:
            ...
            for r in reader:
                if not any(
                    "code_" + k in r and r["code_" + k].strip()
    ...
    if args.get("output"):
        with openio(args["output"], "w") as f:
            writer = csv.DictWriter(
                ...
                + [
                    "code_" + k
                    for k in (fields if fields is not None else CodeResult._fields)
                ],
            )
            writer.writeheader()
            for r in results:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Find code size at the function level.", allow_abbrev=False
    )
